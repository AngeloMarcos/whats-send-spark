...
import { Phone, Star, MapPin, Globe, ExternalLink, Check, Users, Mail, Send } from 'lucide-react';
...
export function AutoSearchResultsTable({ 
  ...
  const getPrimeiroTelefoneSocio = (socios: Socio[] | undefined, nomeEmpresa: string): { telefone: string; nomeSocio: string; link: string } | null => {
    if (!socios || socios.length === 0) return null;
    
    for (const socio of socios) {
      if (socio.tipo !== 'PF') continue;
      if (!socio.telefonesEncontrados || socio.telefonesEncontrados.length === 0) continue;
      ...
      for (let i = 0; i < socio.telefonesEncontrados.length; i++) {
        ...
        if (cleaned.length >= 10 && socio.tiposTelefones?.[i] === 'celular') {
          return {
            ...
            link: getWhatsAppLinkComMensagem(tel, nomeEmpresa)
          };
        }
      }
                          ...
                          {lead.socios && lead.socios.length > 0 ? (
                            <TooltipProvider>
                              <div className="flex items-center justify-center gap-1">
                                ...
                                {/* Botão WhatsApp Sócio - aparece se tiver telefone válido */}
                                {(() => {
                                  const socioData = getPrimeiroTelefoneSocio(lead.socios, lead.razaoSocial || lead.name);
                                  if (socioData) {
                                    return (
                                      <Tooltip>
                                        <TooltipTrigger asChild>
                                          <a
                                            ...
                                            className="inline-flex items-center justify-center h-7 w-7 rounded-full bg-green-600 hover:bg-green-700 text-white transition-colors"
                                          >
                                            <Send className="h-3.5 w-3.5" />
                                          </a>
                                        </TooltipTrigger>
                                        <TooltipContent side="top">
                                          <p className="text-xs">WhatsApp para {socioData.nomeSocio}</p>
                                          <p className="text-xs text-muted-foreground">{socioData.telefone}</p>
                                        </TooltipContent>
                                      </Tooltip>
                                    );
                                  }
