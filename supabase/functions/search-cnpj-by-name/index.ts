...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      console.error('[search-cnpj-by-name] No authorization header');
      return new Response(
    ...
    if (!supabaseUrl || !supabaseAnonKey) {
      console.error('[search-cnpj-by-name] Missing Supabase configuration');
      return new Response(
    ...
    if (claimsError || !claimsData?.user) {
      console.error('[search-cnpj-by-name] Invalid token:', claimsError?.message);
      return new Response(
    ...
    console.log(`[search-cnpj-by-name] Authenticated user: ${userId}`);
